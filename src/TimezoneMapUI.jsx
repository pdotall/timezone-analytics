 ...
 *
 * GEOJSON SAFETY
 * - Fetch Natural Earth timezones as TopoJSON → convert to GeoJSON.
 * - VALIDATE & SANITIZE into a proper FeatureCollection before rendering.
 * - If anything fails, KEEP SHOWING a small built‑in fallback polygon so the map always renders.
 *
 * UX
 * - Boxed layout. Zones are transparent until clicked. Clicking colors the zone.
 * - Paste crypto addresses (lightweight EVM 0x... / rough Solana base58 only).
 * - Clicking a zone appends a row to the table. Clicking the same zone again
 *   toggles it OFF and removes the most recent row for that zone (LIFO).
 * - Multiple zones can stay selected.
 *
 * No Web3/MetaMask calls. Pure UI.
 */

// ---------- Pure helpers (reused in tests) ----------
...
export const isFeatureCollection = (obj) => !!obj && obj.type === 'FeatureCollection' && Array.isArray(obj.features);
